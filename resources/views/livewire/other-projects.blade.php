<div>
    ...
    $proj->projectstartdate <= now() && $proj->projectenddate >= now();
        ...
        $weekBeforeAct <= now() && $projStartDate> now();
            ...
            return $proj->projectstatus === 'Incomplete' &&
            $proj->projectenddate < now(); }); @endphp <div class="input-container mb-2">
...
<script>
    ...
    const searchInputProject = document.getElementById('searchInputProject');
    document.addEventListener('livewire:load', function() {
        searchBtnProject.addEventListener('click', function() {
            var inputData = searchInputProject.value;
            Livewire.emit('searchInput', inputData);
        });

        searchInputProject.addEventListener('keydown', function(event) {
        ...
        searchInputProject.addEventListener('input', function() {

            var inputValue = searchInputProject.value;
            if (inputValue === '') {
                Livewire.emit('resetData', inputValue);
            }
        });
    });
</script>
