<div>
    <div class="container">
        ...
        <div class="row">
            ...
            <div class="col-md border-end border-bottom divhover">
                <div class="row">
                    <div class="col-10 checkmember" data-name="{{ $member->name . ' ' . $member->last_name }}"
                    ...
                    <div class="col-2">
                        ...
                        <button type="button" class="btn btn-outline-danger fs-5 border float-end"
                            wire:click="unassignMembers('{{ $member->id }}')">
    ...
    @if (Auth::user()->role === 'Admin')
    <div class="btn-group ms-3 mt-2 shadow">
        <button type="button" class="btn btn-sm rounded btn-gold shadow" id="btnAddMember" data-bs-toggle="modal"
    ...
    <div class="modal fade" id="addAssigneeModal" tabindex="-1" aria-labelledby="addAssigneeModalLabel"
        aria-hidden="true">
            ...
            <div class="modal-content">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="form-check ms-3">
                            <input class="form-check-input border border-primary" type="checkbox"
                                id="addmember_{{ $addmember->id }}" name="addmember[]" value="{{ $addmember->id }}">
                            <label class="form-check-label"
                                for="addmember_{{ $addmember->id }}">{{ $addmember->last_name . ', ' . $addmember->name . ' ' . ($addmember->middle_name ? $addmember->middle_name[0] : 'N/A') . '.' }}</label>
                        </div>
                        @endforeach
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-md rounded border border-1 btn-light shadow"
                        id="addMembersCloseButton" data-bs-dismiss="modal">Cancel</button>

                    <button type="button" id="saveMembersButton" class="btn btn-md rounded btn-gold shadow"
                        @if($addmembers->isEmpty())style="display:none"@endif>Add Members</button>
