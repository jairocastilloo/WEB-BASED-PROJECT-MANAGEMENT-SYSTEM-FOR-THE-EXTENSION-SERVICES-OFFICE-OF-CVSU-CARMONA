...
<div class="row divhover p-2 mt-1 ps-1 outputinputdiv d-none">
  ...
  <div class="col-3">
    <button type="button" class="btn btn-danger btn-sm" id="removeoutput-btn">Remove</button>
        ...
        $("#outputform").append(divtoadd);
        $("#outputform")
            .find(".outputnamediv:last")
            .next(".outputinputdiv")
            .find("input")
            .val($(".outputnamediv:last h6").text());
    });
    ...
    function subtaskhasError() {
        ...
        if (subtaskduedate === "") {
            ...
            $("#subtaskduedate")
                .next(".invalid-feedback")
                .find("strong")
                .text("Subtask Due Date is required.");
            hasErrors = true;
        }
        // Validation for Project Start Date
        else if (
            new Date(actsavestartdate) > new Date(subtaskduedate) ||
            new Date(actsaveenddate) < new Date(subtaskduedate)
        ) {
            $("#subtaskduedate").addClass("is-invalid");
            $("#subtaskduedate")
                .next(".invalid-feedback")
                .find("strong")
                .text(
                    "Please ensure that the Subtask Due Date falls within the range of " +
                        new Date(actsavestartdate).toLocaleDateString("en-US", {
                            year: "numeric",
                            month: "short",
                            day: "numeric",
                        }) +
